"""
@Introduce : 同时把GT和预测结果可见在同一张上面。$DT_DIR是预测结果标签地址：
@File      : vis_yolo_gt_dt.py
@Time      : 2021/11/16 21:33
@Author    : luhenghui

同时把GT和预测结果可见在同一张上面。$DT_DIR是预测结果标签地址，必须是和GT同名的标签$ROOT_PATH文件目录：
└── $ROOT_PATH
  ├── 类.txt
  ├── 图片
  └── 标签
    ...
    if gt:
        ...
        with open(gt, 'r') as f:
            ...
            for ann in annotations:
                ...
                # 类别名称显示
                cv2.putText(img, str(cls2label[cls]), (c1[0], c1[1] - 2), 0, tl / 4, color, thickness=tf,
    ...
    if dt:
        with open(dt, 'r') as f:
            ...
            for ann in annotations:
                ...
                if len(ann) == 6:
                    cv2.putText(img, str(round(conf, 2)), (c1[0], c1[1] - 2), 0, tl / 4, color, thickness=tf,
                                lineType=cv2.LINE_AA)
